...
out_dir="/usr/scratch/mayank/nocs_intraswap_rslt/wormhole/multiflit_pkt"
cycles=10000
...
do
for i in 0.002 0.004 0.006 0.008 0.010 0.012 0.014 0.016 0.018 0.020 0.022 0.024 0.026 0.028 0.030 0.032 0.034 0.036 0.038 0.040 0.042 0.044 0.046 0.048 0.050
do
    ./build/Garnet_standalone/gem5.debug -d $out_dir/${num_cores}c/${depth}/${bench_caps[$b]}/tr-${tr}/vnet-${vnet}_vc-${vc}_depth-${depth}/inj-${i} configs/example/garnet_synth_traffic.py --topology=Mesh_XY --num-cpus=${num_cores} --num-dirs=${num_cores} --mesh-rows=8 --network=garnet2.0 --router-latency=$tr --link-width-bits=${link_width_bits} --sim-cycles=$cycles --enable-wormhole=1 --wormhole-buffer-depth=${depth} --inj-vnet=${vnet} --injectionrate=${i} --synthetic=${bench[$b]} &

...
out_dir="/usr/scratch/mayank/nocs_intraswap_rslt/intelShuffle_swapnoc/multiflit_pkt"
cycles=10000
...
do
    ./build/Garnet_standalone/gem5.debug -d $out_dir/${num_cores}c/${bench_caps[$b]}/tr-${tr}/vnet-${vnet}_vc-${vc}_depth-${depth}_inj-${k}/ configs/example/garnet_synth_traffic.py --topology=Mesh_XY --num-cpus=${num_cores} --num-dirs=${num_cores} --mesh-rows=8 --network=garnet2.0 --router-latency=$tr --link-width-bits=${link_width_bits} --sim-cycles=$cycles --enable-wormhole=1 --enable-intraswap=1 --wormhole-buffer-depth=${depth} --intraswap-threshold=2 --inj-vnet=${vnet} --injectionrate=${k} --synthetic=${bench[$b]} &
done
sleep 300
done
